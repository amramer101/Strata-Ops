...
sudo java -jar jenkins-plugin-manager-2.12.14.jar \
  ...
  --plugins configuration-as-code git github workflow-aggregator credentials plain-credentials ssh-credentials sonar slack maven-plugin email-ext timestamper ws-cleanup job-dsl nexus-artifact-uploader ssh-agent dependency-check-jenkins-plugin
